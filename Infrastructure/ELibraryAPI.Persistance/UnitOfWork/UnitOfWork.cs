...
public class UnitOfWork : IUnitOfWork
{
    ...
    public IRefreshTokenReadRepository RefreshTokenRead =>
          (IRefreshTokenReadRepository)_serviceProvider.GetRequiredService(typeof(IRefreshTokenReadRepository));

    public IRefreshTokenWriteRepository RefreshTokenWrite =>
        (IRefreshTokenWriteRepository)_serviceProvider.GetRequiredService(typeof(IRefreshTokenWriteRepository));
    ...
    public IReadRepository<T, TKey> ReadRepository<T, TKey>() where T : class, IEntity<TKey>
    {
        ...
        {
            var customRepo = _serviceProvider.GetService(typeof(IReadRepository<T, TKey>));
            _repositories.Add(type, customRepo ?? new ReadRepository<T, TKey>(_context));
        }
    ...
    public IWriteRepository<T, TKey> WriteRepository<T, TKey>() where T : class, IEntity<TKey>
    {
        ...
        if (!_repositories.ContainsKey(type))
        {
            var customRepo = _serviceProvider.GetService(typeof(IWriteRepository<T, TKey>));
            _repositories.Add(type, customRepo ?? new WriteRepository<T, TKey>(_context));
        }
